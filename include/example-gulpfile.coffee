...
app.desc     = "This is my application"
app.url      = "localhost"
app.devName  = theme.author
app.devURL   = theme.authorUrl
...
gulp.task 'coffeeScripts', ->
  gulp.src(path.source+'/scripts/!(_)*.coffee')
      ...
      .pipe($.uglify({})).pipe gulp.dest(THEME_PATH + '/js/')
...
gulp.task 'javaScripts_vendor', ->
  gulp.src(path.source+'/scripts/vendor/**/*.js')
      .pipe($.include(extension: '.js').on('error', errorHandler('Include *.js')))
      .pipe($.uglify({})).pipe gulp.dest(THEME_PATH + '/js/vendor/')
